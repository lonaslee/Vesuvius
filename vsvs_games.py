...
import re
import aiofile
from features import tictactoe, connectfour, reversi, weiqi, battleship
...
class GameFeatures(commands.Cog):
    ...
    async def battleship(self, ctx: commands.Context, opponent: discord.guild.Member):
        ...
        board = await ctx.send(
            f'```ansi\n\u001b[0m\u001b[1;31mRed: {red.user.display_name}'
            f'\u001b[0m, \u001b[1;34mBlue: {blue.user.display_name}```',
            embed=discord.Embed(
        ...
        try:
            ...
            if err.errperson is red:
                await blue.dmprompt.edit(
                    content='your opponent seems to be afk, game cancelled.'
                )
            else:
            ...
            await ctx.send("you need to have DM's open to be able to play this game!")
            self.ingame.remove(red.user)
            self.ingame.remove(blue.user)
            return
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    await ctx.send(
                        f'game timed out, {player.user.display_name} '
                        'took too long to place ships.'
                    )
    ...
    async def wait_confirm(
        ...
        if game == 'go':
            ...
            game = 'reversi'
        async with aiofile.async_open(self.bot.files['game_log'], 'a') as gl:
            await gl.write(
